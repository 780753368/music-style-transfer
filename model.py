...
class cyclegan(object):
    def __init__(self, sess, args):
        ...
        self.batch_size_mask=args.batch_size_mask
        ...
        self.generatorA2B = generator_resnet
        self.generatorB2A= generator_resnet
        self.criterionGAN = mae_criterion
        self.criterionidt = mae_criterion
        ...
        self.layer_num=int(np.log2(self.options.image_size)) - 3
        self._build_model()
    ...
    def _build_model(self):
        ...
        # Generator: A - B - A
        self.fake_B  = self.generatorA2B(self.real_A, self.options,False,True,name="generatorA2B")
        
      
        self.fake_A_ = self.generatorB2A(self.fake_B, self.options,False,True, name="generatorB2A")
        # Generator: B - A - B
        self.fake_A  = self.generatorB2A(self.real_B, self.options,True,True, name="generatorB2A")
    
        self.fake_B_ = self.generatorA2B(self.fake_A, self.options,True,True, name="generatorA2B")
        #feture:
        self.feture_A_real=feture_grab(self.real_A)
        self.feture_B_fake=feture_grab(self.fake_B)
        self.feture_B_real=feture_grab(self.real_B)
        self.feture_A_fake=feture_grab(self.fake_A)
        # to binary
        ...
        # Discriminator: Fake
        self.DB_fake  = self.discriminatorB(self.fake_B + self.gaussian_noise, self.options,
                                          reuse=False, name="discriminatorB")
        self.DA_fake  = self.discriminatorA(self.fake_A + self.gaussian_noise, self.options,
                                          reuse=False, name="discriminatorA")
        # Discriminator: Real
        self.DA_real  = self.discriminatorA(self.real_A + self.gaussian_noise, self.options, reuse=True,
                                          name="discriminatorA")
        self.DB_real  = self.discriminatorB(self.real_B + self.gaussian_noise, self.options, reuse=True,
                                          name="discriminatorB")
        ...
        self.DA_fake_sample  = self.discriminatorA(self.fake_A_sample + self.gaussian_noise,
                                                 self.options, reuse=True, name="discriminatorA")
        self.DB_fake_sample = self.discriminatorB(self.fake_B_sample + self.gaussian_noise,
                                                 self.options, reuse=True, name="discriminatorB")
        if self.model != 'base':
            # Discriminator: All
            self.DA_real_all = self.discriminatorA(self.real_mixed + self.gaussian_noise, self.options, reuse=False,
                                                  name="discriminatorA_all")
            self.DA_fake_sample_all = self.discriminatorA(self.fake_A_sample + self.gaussian_noise,
                                                         self.options, reuse=True, name="discriminatorA_all")
            self.DB_real_all = self.discriminatorB(self.real_mixed + self.gaussian_noise, self.options, reuse=False,
                                                  name="discriminatorB_all")
            self.DB_fake_sample_all = self.discriminatorB(self.fake_B_sample + self.gaussian_noise,
        ...
        self.perceptual_loss= mse_criterion(self.feture_A_real,self.feture_B_fake)+mse_criterion(self.feture_B_real,self.feture_A_fake)
        self.g_loss = self.g_loss_a2b + self.g_loss_b2a + self.cycle_loss + lambda_p*self.perceptual_loss
        # Discriminator loss
        ...
        self.perceptual_loss_sum = tf.summary.scalar("percetual_loss",self.perceptual_loss)
        self.g_sum = tf.summary.merge([self.g_loss_a2b_sum, self.g_loss_b2a_sum, self.g_loss_sum, self.cycle_loss_sum,self.perceptual_loss_sum])
        ...
        # A - B - A
        self.testB = self.generatorA2B(self.test_A, self.options, True, name="generatorA2B")
        self.testA_ = self.generatorB2A(self.testB, self.options, True, name='generatorB2A')
        # B - A - B
        self.testA = self.generatorB2A(self.test_B, self.options, True, name="generatorB2A")
        self.testB_ = self.generatorA2B(self.testA, self.options, True, name='generatorA2B')
        # to binary
    ...
    def train(self, args):
        ...
        # Learning rate
        self.d_lr = tf.placeholder(tf.float32, None, name='learning_rate')
        self.g_lr = tf.placeholder(tf.float32, None, name='learning_rate')
        # Discriminator and Generator Optimizer
        if self.model == 'base':
            self.d_optim = tf.train.AdamOptimizer(self.d_lr, beta1=args.beta1).minimize(self.d_loss, var_list=self.d_vars)
        else:
            self.d_optim = tf.train.AdamOptimizer(self.d_lr, beta1=args.beta1).minimize(self.D_loss, var_list=self.d_vars)
        self.g_optim = tf.train.AdamOptimizer(self.g_lr, beta1=args.beta1).minimize(self.g_loss, var_list=self.g_vars)
        ...
        dataA = glob.glob('./datasets/{}/train/*.*'.format(self.dataset_A_dir))
        dataB = glob.glob('./datasets/{}/train/*.*'.format(self.dataset_B_dir))
        if self.model == 'partial':
            data_mixed = dataA + dataB
        if self.model == 'full':
            data_mixed = glob.glob('./datasets/JCP_mixed/*.*')
        ...
        for epoch in range(args.epoch):
            ...
            self.if_mask = False if epoch > 2 else True
            # Get the proper number of batches
            batch_idxs = min(min(len(dataA), len(dataB)), args.train_size) // self.batch_size

            # learning rate starts to decay when reaching the threshold
            d_lr = args.d_lr if epoch < args.epoch_step else args.d_lr * (args.epoch-epoch) / (args.epoch-args.epoch_step)
            g_lr = args.g_lr if epoch < args.epoch_step else args.g_lr * (args.epoch-epoch) / (args.epoch-args.epoch_step)
            ...
            for idx in range(0, batch_idxs):
                ...
                batch_image = [load_npy_data(batch_file) for batch_file in batch_files]
                batch_images = np.array(batch_image).astype(np.float32) if epoch > 2 else masking(batch_image).detach().numpy().astype(np.float32)
                ...
                if self.model == 'base':
                    ...
                    fake_A, fake_B, _, summary_str, g_loss_a2b, g_loss_b2a, cycle_loss, perceptual_loss, g_loss = self.sess.run([self.fake_A,
                     self.fake_B, self.g_optim, self.g_sum, self.g_loss_a2b, self.g_loss_b2a,self.cycle_loss,
                     self.perceptual_loss,self.g_loss], feed_dict={self.real_data: batch_images, self.gaussian_noise: gaussian_noise,
                                                 self.d_lr: d_lr,self.g_lr: g_lr})
                    if epoch >2:
                     _, summary_str, da_loss, db_loss, d_loss = self.sess.run([
                           ...
                           feed_dict={self.real_data: batch_images, self.fake_A_sample: fake_A, self.fake_B_sample: fake_B,
                                     self.d_lr: d_lr, self.gaussian_noise: gaussian_noise})

                     print('=================================================================')
                     print(("Epoch: [%2d] [%4d/%4d] time: %4.4f, d_loss: %6.2f, G_loss: %6.2f" %
                          (epoch, idx, batch_idxs, time.time() - start_time, d_loss, g_loss)))
                     print(("++++++++++G_loss_A2B: %6.2f G_loss_B2A: %6.2f Cycle_loss: %6.2f p_loss:%6.2f DA_loss: %6.2f DB_loss: %6.2f" %
                           (g_loss_a2b, g_loss_b2a,cycle_loss,perceptual_loss, da_loss, db_loss)))

                    else:
                     print('=================================================================')
                     print(("Epoch: [%2d] [%4d/%4d] time: %4.4f,  G_loss: %6.2f" %
                          (epoch, idx, batch_idxs, time.time() - start_time, g_loss)))
                     print(("++++++++++G_loss_A2B: %6.2f G_loss_B2A: %6.2f Cycle_loss: %6.2f p_loss:%6.2f " %
                           (g_loss_a2b, g_loss_b2a,cycle_loss,perceptual_loss)))
                else:
                    ...
                    fake_A, fake_B, _, summary_str, g_loss_a2b, g_loss_b2a, cycle_loss, perceptual_loss, g_loss = self.sess.run([
                        self.fake_A,self.fake_B, self.g_optim, self.g_sum, self.g_loss_a2b, self.g_loss_b2a,
                        self.cycle_loss, self.perceptual_loss , self.g_loss], feed_dict={self.real_data: batch_images,
                                                                  self.gaussian_noise: gaussian_noise, self.g_lr: g_lr,
                    ...
                    _, summary_str, da_loss, db_loss, d_loss, da_all_loss, db_all_loss, d_all_loss, D_loss = self.sess.run([
                        ...
                        feed_dict={self.real_data: batch_images, self.fake_A_sample: fake_A, self.fake_B_sample: fake_B,
                                   self.d_lr: d_lr, self.gaussian_noise: gaussian_noise, self.real_mixed: batch_images_mixed})
                    self.writer.add_summary(summary_str, counter)
                    ...
                    print(("++++++++++G_loss_A2B: %6.2f G_loss_B2A: %6.2f Cycle_loss: %6.2f p_loss: %6.2f DA_loss: %6.2f DB_loss: %6.2f, "
                           ...
                           (g_loss_a2b, g_loss_b2a, cycle_loss, perceptual_loss, da_loss, db_loss, da_all_loss, db_all_loss)))
    ...
    def test(self, args):
        ...
        if args.which_direction == 'AtoB':
            sample_files = glob.glob('./datasets/{}/test/*.*'.format(self.dataset_A_dir))
        elif args.which_direction == 'BtoA':
            sample_files = glob.glob('./datasets/{}/test/*.*'.format(self.dataset_B_dir))
        else:
